...
sim_1_gibbs = list()
psm_1_gibbs = list()
rand_VI_1_gibbs = list()
split_merge = list()
psm_1_split_merge = list()
rand_VI_1_split_merge = list()
neal8 = list()
psm_1_neal8 = list()
rand_VI_1_neal8 = list()
smn = list()
psm_1_smn = list()
rand_VI_1_smn = list()
...
for (seed in seed_set) {
  ...
  set.seed(seed)
  sim_1_gibbs[[seed]] = gibbs_mix_con(G=10000,
                                burnin = 5000,
  ...
  psm_1_gibbs[[seed]] = comp.psm(sim_1_gibbs[[seed]]$C)
  
  estim_1_VI_tmp  = mcclust.ext::minVI(psm_1_gibbs[[seed]],method = 'all',cls.draw = sim_1_gibbs[[seed]]$C)
  estim_1_VI_gibbs = estim_1_VI_tmp$cl['best',]
  rand_VI_1_gibbs[[seed]] = mcclust::arandi(estim_1_VI_gibbs,data_list_1[[seed]]$groundTruth)
  
  split_merge[[seed]] = run_markov_chain(data = data_list_1[[seed]]$data, 
                                   attrisize = m, 
                                   gamma = gam, 
                                   v = u, 
                                   w = v, 
                                   ...
                                   neal8 = FALSE,
                                   split_merge = TRUE)
  C_sm <- matrix(unlist(lapply(split_merge[[seed]]$c_i, function(x) x + 1)), 
              nrow = 10000, 
              ncol = nrow(data_list_1[[seed]]$data), 
              byrow = TRUE)
  psm_1_split_merge[[seed]] = comp.psm(C_sm)
  estim_1_VI_tmp  = mcclust.ext::minVI(psm_1_split_merge[[seed]],method = 'all',cls.draw = C_sm)
  estim_1_VI_split_merge = estim_1_VI_tmp$cl['best',]
  rand_VI_1_split_merge[[seed]] = mcclust::arandi(estim_1_VI_split_merge,data_list_1[[seed]]$groundTruth)

  neal8[[seed]] = run_markov_chain(data = data_list_1[[seed]]$data, 
                                         attrisize = m, 
                                         gamma = gam, 
                                         v = u, 
                                         w = v, 
  ...
  C_n <- matrix(unlist(lapply(neal8[[seed]]$c_i, function(x) x + 1)), 
              ...
              ncol = nrow(data_list_1[[seed]]$data), 
              byrow = TRUE)
  psm_1_neal8[[seed]] = comp.psm(C_n)
  estim_1_VI_tmp  = mcclust.ext::minVI(psm_1_neal8[[seed]],method = 'all',cls.draw = C_n)
  estim_1_VI_neal8 = estim_1_VI_tmp$cl['best',]
  rand_VI_1_neal8[[seed]] = mcclust::arandi(estim_1_VI_neal8,data_list_1[[seed]]$groundTruth)
  
  smn[[seed]] = run_markov_chain(data = data_list_1[[seed]]$data, 
                                   attrisize = m, 
                                   gamma = gam, 
                                   v = u, 
                                   w = v, 
  ...
  C_smn <- matrix(unlist(lapply(smn[[seed]]$c_i, function(x) x + 1)), 
                ...
                ncol = nrow(data_list_1[[seed]]$data), 
                byrow = TRUE)
  psm_1_smn[[seed]] = comp.psm(C_smn)
  estim_1_VI_tmp  = mcclust.ext::minVI(psm_1_smn[[seed]],method = 'all',cls.draw = C_smn)
  estim_1_VI_smn = estim_1_VI_tmp$cl['best',]
  rand_VI_1_smn[[seed]] = mcclust::arandi(estim_1_VI_smn,data_list_1[[seed]]$groundTruth)
...
sim_1 = cbind(unlist(HD_rand),k_modes_minusOne_rand,k_modes_rand,k_modes_plusOne_rand,unlist(rand_VI_1_gibbs),unlist(rand_VI_1_neal8),unlist(rand_VI_1_split_merge),unlist(rand_VI_1_smn))
colnames(sim_1) = c('HD','K-Modes[2]',"K-Modes[3]","K-Modes[4]",'HMM', 'NEAL8','SPLIT&MERGE', 'SPLIT&MERGE+NEAL8')
sim_1
cat('Saving results \n');cat('\n')
