...
myplotpsm_gt <- function(psm, ground, classes, ax = TRUE, ay = TRUE, 
  ...
  gt_colors <- setNames(combined_palette[1:n_classes], unique(ground))
...
myplotpsm_gt_sep <- function(psm, ground, classes, ax = TRUE, ay = TRUE, gt = 1,
                         cex.Row = 0.8, cex.Col = 0.8, method = "complete", ...) {
  library(pheatmap)
  library(RColorBrewer)
...
myplotpsm_gt_trial <- function(psm, ground, classes, ax = TRUE, ay = TRUE, 
                         cex.Row = 0.8, cex.Col = 0.8, method = "complete", ...) {
  library(pheatmap)
  library(RColorBrewer)
  ...
  # Order rows/columns based on classes and ground truth
  ord <- unlist(lapply(unique(classes), function(cls) {
    indices <- which(classes == cls)       # Get indices for this class
    indices[order(ground[indices])]        # Order those indices by ground truth
  }))
  ...
  # Create annotations for GT and VI
  annotation_col <- data.frame(
    GT = factor(ground[ord]),
    VI = factor(classes[ord])
  )
  ...
  # Ensure enough colors for the number of classes
  n_classes <- length(unique(ground))
  n_vi_classes <- length(unique(classes))
  if (max(n_classes, n_vi_classes) > length(combined_palette)) {
    stop("Not enough unique colors available in combined palette")
  }

  # Map GT to VI to align colors
  gt_to_vi_mapping <- setNames(unique(classes[ord]), unique(ground[ord]))

  # Assign colors for GT and VI based on the mapping
  gt_colors <- setNames(combined_palette[1:n_classes], unique(ground[ord]))
  vi_colors <- gt_colors[gt_to_vi_mapping]  # Align VI colors with GT
